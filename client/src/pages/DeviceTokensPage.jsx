...
const DeviceTokensPage = () => {
    ...
    const handleQuickRegister = async (e) => {
        ...
        try {
            ...
            }, { headers: { Authorization: `Bearer ${token}` } }); // Assuming register endpoint might need auth or is public. The backend route currently needs auth if protected by middleware in blogRoutes logic logic, but pushRoutes assumed protected for management. Registration is usually public, but this manual entry is dashboard side.
        ...
        <div className="min-h-screen bg-gray-50 dark:bg-slate-900 pb-12">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
                <div className="text-center mb-10">
                    <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Device Tokens</h1>
                    <p className="mt-2 text-gray-600 dark:text-gray-400">Manage registered devices for push notifications</p>
                </div>

                {/* Quick Register */}
                <div className="max-w-2xl mx-auto mb-8 bg-white dark:bg-slate-800 p-6 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm">
                    ...
                    <form onSubmit={handleQuickRegister} className="flex gap-4">
                        <input
                            type="text"
                            className="flex-1 px-4 py-2 rounded-lg border border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-gray-900 dark:text-white"
                                ...
                                {tokens.length === 0 ? (
                                    <tr>
                                        <td colSpan="4" className="px-6 py-10 text-center text-gray-500">
                                            No tokens found. Register one from the mobile app or use Quick Add.
                                        </td>
                                    </tr>
                                ) : (
                                    tokens.map((t) => (
                                        <tr key={t.id || t.token} className="hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors">
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-3">
                                                    <div className="font-mono text-xs text-gray-600 dark:text-gray-300 max-w-[200px] truncate" title={t.token}>
                                                        {t.token}
                                                    </div>
                                                    <button onClick={() => copyToClipboard(t.token)} className="text-gray-400 hover:text-indigo-600">

    ...
    return (
        <section className="py-24 bg-transparent" id="pricing">
            <div className="max-w-7xl mx-auto px-6">
                ...
                <div className="mb-16 max-w-2xl">
                    ...
                    <div className="flex items-center gap-4 mt-8">
                        ...
                        <button onClick={() => setYearly(v => !v)}
                            ...
                            <div className={`absolute top-0.5 w-5 h-5 rounded-full bg-transparent transition-transform duration-300 ${yearly ? 'translate-x-6' : 'translate-x-0.5'}`} />
                        </button>
                    ...
                    {plans.map((plan, idx) => (
                        <motion.div
                            ...
                            style={{
                                ...
                                border: `1px solid ${plan.popular ? T : plan.contactSales ? '#e5e7eb' : '#1E1E1E'}`,
                                boxShadow: plan.popular ? `0 0 48px 0 ${T}12` : 'none',
                            }}
                        >
                            {plan.popular && (
                                <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-4 py-1 text-xs font-extrabold uppercase tracking-widest rounded-sm whitespace-nowrap"
                            ...
                            <div className="mb-6">
                                <h3 className="text-lg font-extrabold text-black mb-1">{plan.name}</h3>
                                <p className="text-sm text-gray-500 mb-5 min-h-[48px]">{plan.desc}</p>
                                ...
                                {plan.monthly === null ? (
                                    ...
                                    <>
                                        <div className="flex items-baseline gap-1">
                                            <span className="text-xl font-extrabold text-black/50" style={{ fontFamily: "'DM Mono',monospace" }}>₹</span>
                                        ...
                                        {plan.monthly > 0 && (
                                            <p className="mt-1 text-xs" style={{ color: '#000', fontFamily: "'DM Mono',monospace" }}>
                            ...
                            <ul className="space-y-2.5 mb-8 flex-1">
                                ...
                                {plan.missing.map((f, i) => (
                                    <li key={i} className="flex items-start gap-2.5 opacity-25">
                                        <X size={12} className="mt-0.5 flex-shrink-0 text-black/30" />
                            ...
                            <Link
                                to={plan.ctaLink}
                                className="w-full py-3 rounded text-xs font-extrabold transition-all duration-200 flex items-center justify-center gap-2 uppercase tracking-widest"
                                style={plan.popular
                ...
                {/* Bottom note */}
                <p className="text-center text-xs mt-10" style={{ color: '#000', fontFamily: "'DM Mono',monospace" }}>

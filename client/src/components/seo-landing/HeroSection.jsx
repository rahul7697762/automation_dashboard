...
export default function HeroSection() {
  ...
  useEffect(() => {
    ...
    const obs = new IntersectionObserver(
      ([e]) => {
        if (e.isIntersecting && !manualPause) {
          // Try unmuted first; if browser blocks it, fall back to muted
          video.muted = false;
          video.play().catch(() => {
            video.muted = true;
            setMuted(true);
  ...
  return (
    <section className="relative flex flex-col items-center overflow-hidden grid-bg pt-28 pb-20 px-4">
      {/* Background orbs */}
      <div className="absolute w-[700px] h-[700px] rounded-full bg-brand/10 -top-48 left-1/2 -translate-x-1/2 blur-3xl pointer-events-none" />
      <div className="absolute w-[400px] h-[400px] rounded-full bg-brand/6 bottom-0 right-0 blur-3xl pointer-events-none" />
      <div className="absolute w-[300px] h-[300px] rounded-full bg-brand/4 bottom-20 left-0 blur-3xl pointer-events-none" />
        ...
        >
          ...
          >
            ...
            <button
              ...
              className="absolute top-3 right-3 z-10 flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold backdrop-blur-sm transition-all hover:scale-105"
              style={{
                background: muted ? 'rgba(38,206,206,0.15)' : 'rgba(38,206,206,0.3)',
